                        ...
                        <div class="row mt-0 justify-content-center">
                            <div class=" col-11 col-md-10 pb-3">
                                ...
                                <form method="POST" action="{{route('user.updated',['user'=>$userData->id])}}">
                                    @csrf
                                    @method('PUT')
                                    <input type="hidden" name="userId" id="userId" wire:model= "userId" value="{{$userData->id}}">
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label for="dni" class="text-dark">{{ __('Cédula') }}</label>
                                                <input type="numeric" class="form-control mb-2 @error('dni') is-invalid @enderror" value="{{ $userData->dni }}" required autocomplete="dni" autofocus  maxlength="9"  title="la cédula debe ser solo numerica" pattern="[0-9]+" readonly>
                                            @error('dni')
                                    ...
                                    <div class="form-group">
                                        <label for="selectedUser" class="text-dark">{{ __('Tipo de Usuario') }}</label>
                                            <select wire:model="selectedUser" id="selectedUser" name="user_type" class="custom-select bg-white form-control border-primary @error('user_types') is-invalid @enderror">
                                                    <option value="" >Seleccione</option>
                                                @foreach ($user_types as $user_type)
                                                    <option value="{{ $user_type->id }}" {{ ($user_type->id == $userData->user_type) ? 'selected' : '' }}>{{ $user_type->description }}</option>
                                                @endforeach
                                            </select>
